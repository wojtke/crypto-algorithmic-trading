...
def separate(df):
    ...
    for window in windows:
        ...
        #parts of train set and val set overlap by 1 row, should not be a major leak problem, but if it is, it needs to be fixed
        df_t = df_t.append(df.iloc[df_v.index[-1]:ixs[0]+1])
        df_v = df_v.append(df.iloc[ixs[0]:ixs[1]+1])

        print(datetime.fromtimestamp(df['openTimestamp'][ixs[0]]/1000), "-", datetime.fromtimestamp(df['openTimestamp'][ixs[1]]/1000))
...
def index_window(end, target_time, delta_time):#
    target_ix = round(len(df.index)-(end/1000-datetime.timestamp(target_time))/iv_sec[INTERVAL]) #unprecise because of missing candles
...
def create_seqs(df, which):
    ...
    for i in range(len(df.index)): 
        ...
        sliding_window.append([n for n in values[i][:-1]]) #adds single row of everything except target to sliding window, target is added at last
